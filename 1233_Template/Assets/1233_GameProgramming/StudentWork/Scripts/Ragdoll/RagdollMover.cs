	...
	public class RagdollMover : FimpossibleComponent
	{
		...
		protected virtual void Update()
		{
			...
			if (UpdateInput && updateMovement)
			{
				...
				_moveDirectionLocal = Vector2.zero;

				if (Input.GetKey(KeyCode.A)) _moveDirectionLocal += Vector2.left;
				else if (Input.GetKey(KeyCode.D)) _moveDirectionLocal += Vector2.right;

				if (Input.GetKey(KeyCode.W)) _moveDirectionLocal += Vector2.up;
				else if (Input.GetKey(KeyCode.S)) _moveDirectionLocal += Vector2.down;
			...
			if (Mecanim)
				if (moving)
					...
					Mecanim.SetFloat(Speed, Rigb.velocity.magnitude);
